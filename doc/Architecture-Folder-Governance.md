...
> 定位：长期治理标准（What is allowed / not allowed），不替代 `doc/Roadmap.md` 的任务排期。  
...
- 超过 `1200` 行视为高风险，必须拆分并加入 Roadmap/计划任务。
...
1. `npm run verify`
2. 文档联动排查（README / standards / roadmap / relevant plans）
