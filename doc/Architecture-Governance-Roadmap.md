...
>
> **归档说明（2026-02-26）**：AG/P0-0..3、P1-0..3、P2-0..3 共 12 项已全部完成并合并。A 轨（P0-1/3/2、P1-0/1、P2-3）与 B 轨（P0-0、P1-2/3、P2-1/0/2）双轨并行交付。完成记录以 Git 历史为准。
...
> **说明**：以下为路线图制定时的风险项。AG/P0-0..3、P1-0..3、P2-0..3 已全部完成（2026-02-26 合并），对应风险已闭环。保留本段供后续新增任务参考。

### P0（阻塞级）— 已闭环

- ~~`AG/P0-0` 计划文档更新规则~~ → done  
- ~~`AG/P0-1` direct-write gate 注册表~~ → done  
- ~~`AG/P0-2` workspace 版本声明~~ → done  

### P1（高优先）— 已闭环

- ~~`AG/P1-0` 计划命名体系分叉~~ → done（P1-3 映射）  
- ~~`AG/P1-1` CLI 单文件过大~~ → done（P1-0/1 模块化）  
- ~~`AG/P1-2` Persona Package 分叉~~ → done  

### P2（优化级）— 已闭环

- ~~`AG/P2-0` core 扁平化~~ → done（batch 1：guards 分层）  
- ~~`AG/P2-1` 导出面过宽~~ → done（persona_write_lock 收敛）  
- ~~`AG/P2-2` 运行态/报告态治理~~ → done
...
- 状态：`done`（2026-02-26，batch 1：6 guards 迁入 guards/）
